...
Set your identity in `libs/email_review/src/email_review/account.py` (your
...
7. **Render.** The digest is rendered by the `email-review` web service (a
   FastAPI app; see `libs/email_review`). Run `classify.py` to write
   `data/.apps/email-review/data.json`, then the web service reads it and renders
   the bucket-grouped triage UI. The Refresh / Categorize buttons in that UI
   re-run this pipeline.

## One-click actions (the email-review web UI)
