---
name: welcome
description: Greet the user when a new project starts. This mind was created from the "Inbox Digest & Review" inspiration, so the welcome introduces that inspiration and immediately starts the adaptation conversation.
---

# Welcome the user (inspiration: Inbox Digest & Review)
...
- Manifest: `inspiration-inbox-digest-review.md` (at the repo root)

Do ALL of the following in your FIRST response, in the same turn, without
waiting to be asked:

1. Open with a short CUSTOM welcome that names **Inbox Digest & Review** and gives the
   one-line description above. Do NOT use a generic "Welcome to Minds"
   greeting and do NOT offer a generic suggestions list.
2. Immediately read `inspiration-inbox-digest-review.md` at the repo root (reading the
...
3. In plain, non-technical language, present what the inspiration is and
   ...
   up to their own Gmail now (e.g. "Want me to connect this to your Gmail and
   set it up?"). End your first response on THAT question. This is the
   ...
   each `requires_permission` via a latchkey permission request, then (still
   part of activation, before their first look) set their identity in
   `account.py` and run `contact_audit.py` to populate their contacts so the
   first digest is correctly sorted rather than all-cold, get the app showing
   THEIR OWN DATA (that is the definition of working; a running service is
   not), invite them to take a look -- and only then ask how they want to
   adapt it.
